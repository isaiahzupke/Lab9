 ...
 * same first character as a designated word. This class splits the list into a parallel stream then
 * binds the stream to a list to return.
 */
public class ParallelStreamSearch implements AutoCompleter {
     ...
     */
    public ParallelStreamSearch(List<String> list, Logger logger){
        this.list = list;
        this.logger = logger;
    }
    ...
    public List<String> allThatBeginWith(String prefix) {
        ...
        returnList = list.parallelStream().filter(x -> x.startsWith(prefix)).collect(Collectors.toList());
        return returnList;
    }
